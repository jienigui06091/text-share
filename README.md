...
临时文本与文件中转站。创建房间后，把链接发送到另一台电脑或手机；文本会在最后一次更新后的 24 小时删除，上传至 R2 的文件会保留。
...
`R2_PUBLIC_BASE_URL` 是文件公开访问基址，例如 `https://miaowazz.cc.cd/text-share`。配置后，文件会按房间名保存到 `${R2_PREFIX}/files/<房间名>/<随机 ID>-<原文件名>`；预览和下载会访问对应的 `${R2_PUBLIC_BASE_URL}/files/<房间名>/<随机 ID>-<原文件名>`。该前缀中的文件必须允许公开读取。未配置时，本地运行会回退到应用接口代理下载。
